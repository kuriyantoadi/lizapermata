...
if ($_GET['action'] == "table_data") {
    $columns = array( 
        ...
        4=> 'total_harga',
        5=> 'nama', // Dari tabel user
        6=> 'plg.nama', // Ambil nama dari tabel pelanggan
        7=> 'kode_keranjang',
    );
    ...
    if (empty($_POST['search']['value'])) {            
        $query = $koneksi->query("SELECT *, plg.nama AS nama_pelanggan FROM user AS usr 
        ...
        $querycount = $koneksi->query("SELECT count(id_transaksi) as jumlah FROM user AS usr 
            RIGHT JOIN transaksi AS trs ON trs.oleh=usr.id_user 
            LEFT JOIN tb_pelanggan AS plg ON trs.id_pelanggan = plg.id_pelanggan 
            WHERE trs.kode_keranjang LIKE '%$search%' OR trs.total_harga LIKE '%$search%'");
        $datacount = $querycount->fetch_array();
    ...
    if (!empty($query)) {
        ...
        while ($r = $query->fetch_array()) {
            ...
            $nestedData['nama']                 = $r['nama']; // Dari tabel user
            $nestedData['nama_pelanggan']       = $r['nama_pelanggan']; // Dari tabel pelanggan
            $nestedData['aksi']                 =  '
                                                    <center>
